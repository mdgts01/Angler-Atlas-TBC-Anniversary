...
local mapFishIconSize = 30
local mapFishIconStep = 33
local mapFishPanelWidth = 46
...
local function GetZoneIDFromMapID(mapID)
    ...
    if not zoneID then
        if mapID == 947 or mapID == 1414 or mapID == 1415 or mapID == 1945 or mapID == 1949 or mapID == 1955 then
            return
        end
...
function MapFishRates:Create()
    ...
    mapFishRates:SetSize(mapFishPanelWidth, 408)
    mapFishRates:SetPoint("TOPLEFT", map, "TOPRIGHT", -5, -26)

    local modeButtonSize = mapFishIconSize
    local flareModSize = 1.4
    ...
    mapFishRates.modeButton:SetPoint("TOP", mapFishRates, "TOP", 0, -5)
    ...
    for i = 1, 14 do
        ...
        fishIcon:SetSize(mapFishIconSize, mapFishIconSize)
        fishIcon:SetPoint("TOP", mapFishRates, "TOP", 0, -42 - (i - 1) * mapFishIconStep)
        ...
        _G[fishIcon:GetName().."NormalTexture"]:SetSize(mapFishIconSize*1.662, mapFishIconSize*1.662)
    ...
    fishHighlight:SetSize(mapFishIconSize, mapFishIconSize)
...
function MapFishRates:Update()
    if not mapFishRates or not mapFishRatesText then
        return
    end
    ...
    mapFishRates:SetSize(mapFishPanelWidth, 46 + mapFishIconStep * #sortedFish)
